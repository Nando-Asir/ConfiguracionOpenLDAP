...
2.  **Generación de Contraseña Encriptada**
    Para el usuario, necesitarás la contraseña encriptada (puedes usar SSHA o MD5 como indicaste).
...
3.  **Creación del usuario (`usuario.ldif`)**
    Crea el usuario `testuser` (equivalente a `juan` en tu práctica) con UID y GID coincidentes.
...
4.  **Preparación del Directorio Home (Manual)**
    Este paso manual asegura que el directorio exista en el servidor antes de la autenticación del cliente (siguiendo tu práctica).
